...
// Category.querySecondCategory = function (id,callback) {
//     var selectSql = 'select * from nav where categoryId=?';
//     db.query(selectSql,[id],function (err, result) {
//         if (err) {
//             return callback(err);
//         }
...
// };
//查询面部护理类
Category.queryFaceList = function (callback) {
    var selectSql = 'select * from facelist where categoryId=?';
    db.query(selectSql, [1], function (err, result) {
        if (err) {
            console.log(err)
            return callback(err);
        }
...
// Category.queryFace = function (id,callback) {
//     var selectSql = 'select * from face where hot=1';
//     db.query(selectSql,[id],function (err, result) {
//         if (err) {
//             return callback(err);
//         }
